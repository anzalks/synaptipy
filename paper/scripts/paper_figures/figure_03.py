...
def main():
    ...
    markers_map = {"0021 spike_detection": "o", "0022 event_detection": "s"}
    ...
    for col_idx, label in enumerate(datasets):
        ...
        ax_time.bar(
            ...
            overhead_component,
            BAR_WIDTH,
            bottom=compute_component,
            label="I/O & Overhead (Bar)",
    ...
    if rend_res and e2e_res:
        ...
        if sw_rend_med and sw_e2e_med:
            ...
            style_line_axis(
                ax_rend,
                xlabel="Overlaid Trials (N)",
                ylabel="Latency (ms)",
                xticks=TARGET_N,
                xticklabels=[str(n) for n in TARGET_N],
            )
