...
*Figure 3: Computational Performance and Rendering Benchmarks. **(A-D)** High-throughput batch execution scaling across increasing CPU core counts. **(A)** Elapsed wall-clock time for purely I/O-bound operations. The upward trend indicates concurrent disk access creates an I/O bottleneck. **(B)** The measured speedup for the I/O-bound task (blue) collapses below the ideal linear scaling threshold (dashed line). **(C)** Elapsed wall-clock time for heavily CPU-bound analytical processing (e.g., event detection). **(D)** The speedup curve for the CPU-bound task (red) demonstrates strong near-linear scaling up to 6 physical cores. **(E, F)** Rendering optimizations and GUI responsiveness. **(E)** Line plot comparing per-frame update times (ms) between experimental hardware-accelerated backend (blue) and default software rendering backend (red) across overlapping trials (N). **(F)** Bar chart highlighting median update times. The default software rendering maintains update times well below the 16.6 ms threshold for a smooth 60 Hz display. **(G, H)** End-to-end rendering stability. **(G)** Internal `_update_plot()` generation overhead. **(H)** Overall frame time (ms), which includes both plot updates and UI event processing. Highly optimized software rendering provides superior, jitter-free performance for standard 2D electrophysiological timeseries data.*
