...
For active properties, SynaptiPy utilized a standard derivative-crossing threshold (dV/dt > 20 V/s) to identify action potential onset. SynaptiPy demonstrated functionally equivalent extractions with both benchmarks for primary metrics such as AP threshold, amplitude, and half-width (Table 1). However, secondary metrics exhibited a mixed correlation pattern: ADP amplitude (r = -0.3867, ns vs. IPFX), Mean Firing Frequency, and Spike Frequency Adaptation showed variance against specific benchmarks. These discrepancies highlight diverse mathematical paradigms: while IPFX utilizes a 9.9 kHz Bessel filter and eFEL employs bounded derivative stencils, SynaptiPy applies a dynamic 0.1 ms temporal smoothing window, placing its derivative scaling between the two benchmarks.

For subthreshold passive properties, SynaptiPy aligned closely with eFEL and IPFX for Resting Membrane Potential (r > 0.98) (Table 2). Conversely, Input Resistance showed mixed results (strong against eFEL; weak, non-significant against IPFX). Furthermore, Membrane Time Constant (τm) correlations were non-significant against both benchmarks, largely due to the highly reduced effective sample size (n = 12) surviving SynaptiPy's strict biological fit-quality gate (R² ≥ 0.80). The +15.58 bias observed in Sag Ratio represents a unit definitional mismatch (e.g., percentage versus 0-1 fractional ratio) rather than a physiological extraction failure.
...
## Biological Use-Case Demonstration: High-Throughput Cell Classification
Beyond computational parity, SynaptiPy offers immediate utility for experimental applications. To demonstrate this in a live neuroscience workflow, SynaptiPy was utilized to classify cortical networks by discriminating fast-spiking (PV+) interneurons from excitatory pyramidal cells based on distinct ion channel kinetics. A directory of n = 54 unlabeled cell recordings was loaded into the Explorer environment. A unified derivative threshold was visually verified on a single representative cell, and the `BatchAnalysisEngine` was executed across the entire cohort. 

This automated execution mathematically separated the cohort into two distinct populations: a cluster of putative PV+ fast-spiking interneurons (n = 16) characterized by compressed action potential half-widths (< 0.5 ms) and pronounced AHP depths (> 12 mV) driven by dense Kv3 channel expression, distinguishing them completely from broader-spiking excitatory pyramidal cells (n = 38; half-width > 0.8 ms) (Figure 4). This workflow condenses manual cursor-placement in commercial software into a single, automated 5-second execution block.

*(Insert Figure 4 placeholder: Scatter plot of AP Half-Width vs AHP Depth showing distinct PV+ vs Pyramidal clustering).*

## High-Throughput Processing and Rendering Optimization
End-to-end benchmarking indicates that the software maintains stable GUI execution times even as the complexity of multi-channel recordings scales. Full analytical processing completes in approximately 3.3 ms per recording. Under software rendering, the full end-to-end application loop (including UI event dispatch) ranges from 4.7 to 12.8 ms for N = 10–20 overlaid trials, remaining comfortably below the 16.6 ms threshold required for smooth 60 Hz rendering. Under OpenGL acceleration, raw canvas latency scales from 1.0 to 5.2 ms, reflecting GPU parallelism advantages for large trial overlays.
...
| **Scriptable Batch Engine** | Yes (Python) | Limited (Macros) | Yes (C++) | Yes (Python) | Yes (Python) |
...
While SynaptiPy exhibits strong linear correlations against eFEL and IPFX for core properties, nuanced mathematical definitions drive discrepancies in secondary metrics. The non-significant correlation observed for the Membrane Time Constant (τm) is heavily influenced by SynaptiPy's strict quality gating (R² ≥ 0.80); un-fittable traces containing biological noise were rigorously rejected, reducing the effective N to 12. Similarly, the pronounced mean bias (+15.58) in Sag Ratio highlights an important unit mismatch across libraries—SynaptiPy computes sag as a raw percentage, whereas standard computational libraries frequently calculate it as a 0-1 fractional ratio. Future iterations will standardize these absolute output scales to match community defaults.
