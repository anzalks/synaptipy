...
*Figure 4: Rendering optimizations for high-frequency multichannel data. **(A)** Absolute per-frame update latency as the number of overlaid trial sweeps (N) increases, comparing standard software rasterization (QPainter) to hardware-accelerated OpenGL (Metal). Error bars represent the 5th and 95th percentiles of recorded rendering latency. **(B)** Grouped bar chart comparing rendering times directly; OpenGL acceleration significantly attenuates latency scaling at maximum plotting densities.*
