...
try:
    ...
    class DummyNeoAdapter:
        def get_supported_file_filter(self):
            return "Dummy Files (*.dummy);;All Files (*)"
        ...
        def read_recording(self, filepath):
            ...
            if not Path(filepath).exists():
                try:
                    ...
                    log.warning(f"Could not create dummy file {filepath}: {e}")
            fname_lower = str(filepath).lower()
            if "5ch" in fname_lower:
                num_chan = 5
            elif "1ch" in fname_lower:
                num_chan = 1
            else:
                num_chan = 3
    ...
    class DummyNWBExporter:
        def export(self, recording, output_path, metadata):
            log.debug(
                f"DummyNWBExporter: Simulating export of '{recording.source_file.name}' to {output_path} with metadata {metadata}"
            )
            try:
                output_path.touch()
                ...
                class DummyExportError(Exception):
                    pass
    ...
    def DummyErrors():
        ...
        class FileReadError(SynaptipyError):
            pass

        class UnsupportedFormatError(SynaptipyError):
            pass

        class ExportError(SynaptipyError):
            pass

        class SynaptipyFileNotFoundError(SynaptipyError):
            pass
...
if "NeoAdapter" not in locals():
    NeoAdapter = DummyNeoAdapter  # Ensure NeoAdapter exists even if only dummies defined
if "NWBExporter" not in locals():
    NWBExporter = DummyNWBExporter
if "Recording" not in locals():
    Recording = DummyRecording
if "SynaptipyError" not in locals():
    SynaptipyError, FileReadError, UnsupportedFormatError, ExportError, SynaptipyFileNotFoundError = DummyErrors()
