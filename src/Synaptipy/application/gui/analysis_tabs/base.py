...
class BaseAnalysisTab(QtWidgets.QWidget, ABC, metaclass=QABCMeta):
    ...
    # --- ADDED: Method to setup batch button ---
    def _setup_batch_button(self, layout: QtWidgets.QLayout, alignment=QtCore.Qt.AlignmentFlag.AlignCenter):
        """Adds a 'Batch Process...' button to the layout."""
        self.batch_button = QtWidgets.QPushButton("Batch Process...")
        self.batch_button.setToolTip("Open Batch Analysis Dialog with current settings applied to all files.")
        self.batch_button.setIcon(QtGui.QIcon.fromTheme("system-run"))
        
        style_button(self.batch_button)
        self.batch_button.clicked.connect(self._on_batch_button_clicked)
        
        if alignment:
            layout.addWidget(self.batch_button, 0, alignment)
        else:
            layout.addWidget(self.batch_button)
        log.debug(f"{self.__class__.__name__}: Batch button setup.")

    @QtCore.Slot()
    def _on_batch_button_clicked(self):
        """
        Handles batch button click.
        Gathers current parameters and opens BatchAnalysisDialog.
        """
        log.info(f"{self.__class__.__name__}: Batch process requested.")
        
        # 1. Gather current parameters
        try:
            params = self._gather_analysis_parameters()
        except Exception as e:
            log.error(f"Failed to gather parameters for batch: {e}")
            QtWidgets.QMessageBox.warning(self, "Batch Error", f"Could not gather current parameters:\n{e}")
            return

        if not params:
            QtWidgets.QMessageBox.warning(self, "Batch Error", "Invalid parameters. Please check your settings.")
            return
        ...
        try:
            ...
            log.error(f"{self.__class__.__name__} does not implement get_registry_name")
            QtWidgets.QMessageBox.warning(self, "Batch Error", "Batch processing not supported for this analysis type yet.")
            return
            
        # 3. Construct pipeline step config
        ...
        step_config = {
            ...
            'scope': 'all_trials', # Default to all trials, user can change
            'params': params
        }
        
        # 4. Get all available files from parent AnalyserTab
        # We need to access the parent AnalyserTab to get the full list of items
        # The parent of this widget is the QTabWidget, whose parent is AnalyserTab
        analyser_tab = None
        current = self.parent()
        while current:
            # Check for _analysis_items and update_analysis_sources to identify AnalyserTab
            # We removed batch_analysis_btn, so we can't check for that anymore
            if hasattr(current, '_analysis_items') and hasattr(current, 'update_analysis_sources'):
                analyser_tab = current
                break
            current = current.parent()
            
        if not analyser_tab:
            log.error("Could not find parent AnalyserTab")
            QtWidgets.QMessageBox.warning(self, "Batch Error", "Could not access file list.")
            return
            
        # 5. Open Batch Dialog
        # We can reuse the _on_batch_analysis_clicked logic but pass the config
        # Import here to avoid circular import
        from Synaptipy.application.gui.batch_dialog import BatchAnalysisDialog
        
        # Collect unique files
        unique_files = set()
        for item in analyser_tab._analysis_items:
        ...
        # 6. Get current channel name if available
        default_channels = None
        if hasattr(self, 'signal_channel_combobox') and self.signal_channel_combobox:
            # Use currentData() to get the actual channel ID (userData), not the display text
            current_channel_id = self.signal_channel_combobox.currentData()
            if current_channel_id is not None:
                default_channels = [str(current_channel_id)]
        
        # Create dialog with pre-filled pipeline
        dialog = BatchAnalysisDialog(files_list, pipeline_config=[step_config], default_channels=default_channels, parent=self)
        dialog.exec()
    # --- END ADDED ---
