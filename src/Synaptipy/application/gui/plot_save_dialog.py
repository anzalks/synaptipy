...
class PlotSaveDialog(QtWidgets.QDialog):
    ...
    def _browse_path(self):
        ...
        directory = QtWidgets.QFileDialog.getExistingDirectory(self, "Select Save Directory", current_path)
    ...
    def _save_plot(self):
        ...
        if not path or not filename:
            QtWidgets.QMessageBox.warning(self, "Invalid Input", "Please provide both a path and filename.")
            return

        if not os.path.exists(path):
            QtWidgets.QMessageBox.warning(self, "Invalid Path", f"The selected path does not exist:\n{path}")
            return
            ...
            try:
                ...
                if painter.begin(printer):
                    try:
                        ...
                        # Draw the pixmap
                        painter.drawPixmap(int(x_offset), int(y_offset), int(scaled_width), int(scaled_height), pixmap)
