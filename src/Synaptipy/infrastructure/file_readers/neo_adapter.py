...
if __name__ == "__main__":
    ...
    log.debug("=" * 30)
    log.debug("Running NeoAdapter example (IODict lookup)...")
    log.debug("=" * 30)
        ...
        try:
            ...
            for ch_id, channel in recording_data.channels.items():
                ...
                if channel.data_trials:
                    print(f"    First Trial Len:   {len(channel.data_trials[0])} samples")
                print("-" * 20)
        except Exception as e:
            print(f"\n--- ERROR during example execution ---")
            log.exception("Exception occurred in example usage block:")
            print(f"Error Type: {type(e).__name__}")
            print(f"Error Details: {e}")
    log.debug("=" * 30)
    log.debug("NeoAdapter example finished.")
    log.debug("=" * 30)
