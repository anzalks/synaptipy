...
def apply_winwcp_patch():  # noqa: C901
    ...
    def patched_parse_header(self):
        ...
        with open(self.filename, "rb") as fid:
            ...
            if "NC" not in header:
                header["NC"] = 1
            if "NBD" not in header:
                header["NBD"] = 1
            if "NBA" not in header:
                header["NBA"] = 0
            if "ADCMAX" not in header:
                header["ADCMAX"] = 1
            ...
            if analysisHeader is None:
                log.warning(
                    "analysisHeader was not populated (NR=0 or read failed). Attempting fallback read at offset 1024."
                )
                try:
