...
class PlotZoomSyncManager(QtCore.QObject):
    ...
    def is_zoom_available(self) -> bool:
        ...
        return self.plot_widget is not None and self.view_box is not None and self.base_x_range is not None

    def is_scroll_available(self) -> bool:
        """Check if scroll controls are available and functional."""
        return self.is_zoom_available() and (self.x_scrollbar is not None or self.y_scrollbar is not None)
    ...
    def _calculate_new_range(self, base_range: Tuple[float, float], slider_value: int) -> Optional[Tuple[float, float]]:
        ...
        try:
            ...
            normalized_zoom = (float(slider_value) - self.SLIDER_RANGE_MIN) / (
    ...
    def _update_scrollbar_from_view(
        self, scrollbar: QtWidgets.QScrollBar, base_range: Tuple[float, float], view_range: Tuple[float, float]
    ):
