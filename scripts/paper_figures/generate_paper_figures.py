...
def run_script(script_path: Path):
    ...
    try:
        ...
        print(f"\nError: Script {script_path.name} failed with exit code {e.returncode}.", file=sys.stderr)
...
def main():
    ...
    parser.add_argument(
        "--run-analysis", 
        action="store_true", 
        help="Run analysis scripts (benchmarks, tables) before plotting figures."
    )
    parser.add_argument(
        "--figures", 
        nargs="+", 
    ...
    print("All figures are saved in paper/results/")
